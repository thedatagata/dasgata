// components/onboarding/SourcesSelection.tsx
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div class="text-center mb-12">
        ...
        <p class="text-xl text-[#F8F6F0]/80">
          Select the tools you want to analyze
        </p>
      </div>
              ...
              return (
                <button
                  ...
                  class={`p-6 rounded-lg border-2 transition-all text-left ${
                    isSelected
                      ? 'bg-[#90C137]/20 border-[#90C137] shadow-lg'
                      : 'bg-[#F8F6F0]/5 border-[#F8F6F0]/30 hover:border-[#90C137]/50'
                  }`}
                >
                  <div class="flex items-start justify-between mb-3">
                    <div class={`w-12 h-12 rounded-lg flex items-center justify-center ${
                      isSelected ? 'bg-[#90C137]' : 'bg-[#F8F6F0]/10'
                    }`}>
                      <i class={`fas ${source.icon} text-xl ${
                        isSelected ? 'text-[#172217]' : 'text-[#90C137]'
                      }`}></i>
                    </div>
                    {isSelected && (
      ...
      <div class="flex justify-between mt-12">
        <button onClick={handleBack} class="px-6 py-3 border border-[#F8F6F0]/30 text-[#F8F6F0] rounded-md hover:bg-[#F8F6F0]/10">
          ← Back
        </button>
        <button
          onClick={handleContinue}
          disabled={selectedSources.length === 0}
          class={`px-8 py-3 rounded-md font-medium ${
            selectedSources.length > 0
              ? 'bg-[#90C137] text-[#172217] hover:bg-[#a0d147] shadow-lg'
              : 'bg-[#F8F6F0]/20 text-[#F8F6F0]/40 cursor-not-allowed'
          }`}
        >
          Get Started →
        </button>
