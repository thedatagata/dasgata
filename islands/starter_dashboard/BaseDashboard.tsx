...
export default function BaseDashboard({ motherDuckToken, sessionId }: BaseDashboardProps) {
  ...
  async function handleGenerateQuery() {
    ...
    try {
      ...
      // Get explanation
      const explainQuery = `SELECT explanation FROM prompt_explain('${
        sql.replace(/'/g, "''")
      }') LIMIT 1`;
            ...
            : (
              <div class="space-y-4">
                {tables.map((table) => (
                  <div
                    key={table.full_path}
                    class="bg-[#F8F6F0]/5 border border-[#90C137]/30 rounded-lg p-4"
                  >
                  ...
                  <thead class="bg-[#F8F6F0]/5">
                    <tr>
                      {tableSummary.length > 0 &&
                        Object.keys(tableSummary[0]).map((key) => (
                          <th
                            key={key}
                            class="px-3 py-2 border border-[#90C137]/30 text-left font-medium text-[#90C137]"
                          >
                            {key}
                          </th>
                        ))}
                  ...
                  <thead class="bg-[#F8F6F0]/5 sticky top-0">
                    <tr>
                      {tablePreview.length > 0 &&
                        Object.keys(tablePreview[0]).map((key) => (
                          <th
                            key={key}
                            class="px-3 py-2 border border-[#90C137]/30 text-left font-medium text-[#90C137]"
                          >
                    ...
                    {tablePreview.map((row, idx) => (
                      <tr key={idx} class="hover:bg-[#F8F6F0]/5">
                        {Object.values(row).map((val, i) => (
                          <td
                            key={i}
                            class="px-3 py-2 border border-[#90C137]/30 text-[#F8F6F0] whitespace-nowrap"
                          >
                            {typeof val === "bigint"
                              ...
                              ? val.toLocaleString()
                              : (val && typeof val === "object" && "days" in val)
                              ? new Date((val as any).days * 24 * 60 * 60 * 1000)
                                .toLocaleDateString()
                              : String(val)}
                          </td>
                        ))}
                      ...
                      {Object.values(row).map((val, i) => (
                        ...
                        >
                          {typeof val === "bigint"
                            ...
                            ? val.toLocaleString()
                            : (val && typeof val === "object" && "days" in val)
                            ? new Date((val as any).days * 24 * 60 * 60 * 1000).toLocaleDateString()
                            : String(val)}
                        </td>
                      ))}
                    </tr>
          ...
          <div class="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <select
                value={chartType}
                onChange={(e) =>
                  setChartType((e.target as HTMLSelectElement).value as "line" | "bar")}
            ...
            <div>
              ...
              >
                ...
                {availableColumns.filter((col) =>
                  !numericColumns.includes(col) && col !== selectedDimensions[0]
